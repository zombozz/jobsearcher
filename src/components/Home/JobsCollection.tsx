    ...
    <div className='min-w-full mx-auto lg:px-20 2xl:px-72 flex gap-20'>
      {!initialLoad && isMobileView && job && (
        <div className='fixed top-0 left-0 right-0 bottom-0 bg-gray-900 bg-opacity-80 z-50 p-4'>
          <div className='bg-base-100 p-6 rounded-lg relative h-full'>
            <button 
            ...
            <>
              ...
              <p className='text-sm border border-w-2 bg-base-200 mt-1 py-1 px-1 rounded-md w-max'>{job.pay}</p>
              <button 
      ...
      <div className='md:w-1/3'>
        {data.map((job) => (
          <div
            ...
            className={`my-10 bg-base-100 p-4 border-2 border-gray-400 card h-56 cursor-pointer ${selectedJobId == job.job_id ? 'border-primary' : ''}`}
          >
            <h3 className='text-xl font-bold'>{job.title}</h3>
            <p className='text-sm'>{job.company}</p>
            <p className='text-sm'>{job.location}</p>
            <p className='text-sm border border-w-2 bg-base-200 mb-3 mt-1 py-1 px-1 rounded-md w-max'>{job.pay}</p>
            <p className='text-sm overflow-hidden'>{job.description}</p>
          </div>
        ))}
      </div>
      ...
      <div className="w-2/3 h-screen overflow-y-auto hidden md:block sticky top-20">
        {job && (
          <div className='my-10 bg-base-100 p-4 border-2 border-gray-400 card h-max'>
            <h3 className='text-xl font-bold'>{job.title}</h3>
            <p className='text-sm'>{job.company}</p>
            <p className='text-sm'>{job.location}</p>
            <p className='text-sm border border-w-2 bg-base-200 mt-1 py-1 px-1 rounded-md w-max'>{job.pay}</p>
